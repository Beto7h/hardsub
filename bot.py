...
# --- FUNCIÓN PARA EXTRAER METADATOS (Miniaturas y Duración) ---
...
def get_config_menu(user_id):
    ...
    # Traducción de términos técnicos para el usuario
    p_name = {"ultrafast": "Rápido ⚡", "veryfast": "Medio 🏃", "slow": "Lento (Mejor) 🐢"}[data['preset']]
    q_name = {"18": "Alta ⭐", "22": "Buena ✅", "28": "Baja 📉"}[data['crf']]

    text = (
        "🎬 **CONFIGURACIÓN DE PROCESADO**\n\n"
        f"🎨 **Color:** `{c_name}` | 📏 **Size:** `{data['size']}px`\n"
        f"🚀 **Velocidad:** `{p_name}`\n"
        f"🎯 **Calidad:** `{q_name}`\n\n"
    ...
    markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚀 Vel: Rápido", callback_data="set_pre_ultrafast"),
         InlineKeyboardButton("🐢 Vel: Lento", callback_data="set_pre_slow")],
        [InlineKeyboardButton("⭐ Cal: Alta", callback_data="set_crf_18"),
         InlineKeyboardButton("✅ Cal: Buena", callback_data="set_crf_22")],
...
async def run_engine(client, status_msg, user_id):
    ...
    if os.path.exists(output):
        # Obtener info para miniatura y barra de tiempo
