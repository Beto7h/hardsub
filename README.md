...
1. **API_ID** y **API_HASH:** Se obtienen registrando una aplicación en [my.telegram.org](https://my.telegram.org).
2. **BOT_TOKEN:** Crea tu bot en Telegram conversando con [@BotFather](https://t.me/BotFather).
3. **DUMP_CHAT_ID:** Crea un canal privado en Telegram, añade a tu bot como Administrador y obtén su ID única (debe empezar con `-100`). El bot usará este canal como almacenamiento temporal.
4. **STRING_SESSION (Opcional):** Si deseas procesar archivos de más de 2 GB (hasta 4 GB), es necesario generar un String Session usando una cuenta con Telegram Premium activo.
